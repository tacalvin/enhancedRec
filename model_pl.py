...
class DQLR(nn.Module):
    ...
    def forward(self, input):
        ...
        print("QUANT_T:{} QUANT_B:{} DIFF:{}".format(
            quant_t.shape, quant_b.shape, diff.shape))
        dec = []
...
class DQLRnn(nn.Module):
    ...
    def forward(self, input):
        ...
        quant_t, quant_b, diff, _, _ = self.encode(
        ...
        out = rearrange(out, '(b f) c w h -> b f c w h',
                        b=batch_size, f=seq_length)
...
class DQLRnnDist(plt.LightningModule):

    def __init__(self, model_config, training_config):
        super(DQLRnnDist, self).__init__()

        self.model = DQLRnn(**model_config)
        self.training_config = training_config
        self.save_hyperparameters()
        self.transform = transforms.Compose(
            [
                transforms.Grayscale(num_output_channels=1),
                transforms.Resize(self.training_config['size']),
                transforms.CenterCrop(self.training_config['size']),
                transforms.ToTensor(),
    ...
    def loss(self, x, latent_loss, y):
        criterion = nn.MSELoss()
        crit_triplet = nn.TripletMarginLoss(
            self.training_config['triplet_margin'],
            p=self.training_config['p'])
        lam_latent = self.training_config['lam_latent']
        lam_triplet = self.training_config['lam_triplet']

        #normalize x, y
        # x /= x.max()
        # y /= y.max()
        recon_loss = criterion(x, y)
        recon_loss /= 3.0  # TODO fix to generalize for more than 3 images

        triplet_loss = crit_triplet(
            x[:, 1, :, :, :], x[:, 0, :, :, :], x[:, 2, :, :, :])
        latent_loss = latent_loss.mean()
        loss = recon_loss + lam_latent * \
            latent_loss + (lam_triplet * triplet_loss)

        return loss, recon_loss, latent_loss, triplet_loss

    def configure_optimizers(self):
        optimizer = optim.Adam(
            self.parameters(), lr=self.training_config['lr'])
        return optimizer

    def training_step(self, batch, batch_nb):
        img, img_s = batch
        out, latent_loss = self.forward(img_s)

        loss, mse_loss, latent_loss, triplet_loss = self.loss(
            out, latent_loss, img_s)
        self.log('training_loss', loss, on_step=True,
                 on_epoch=True, sync_dist=True,)
        self.log('mse_loss', mse_loss, on_step=True, prog_bar=True, on_epoch=True, sync_dist=True)
        self.log('latent_loss', latent_loss, on_step=True, prog_bar=True,
                 on_epoch=True, sync_dist=True)
        self.log('triplet_loss', triplet_loss, on_step=True, prog_bar=True,
                 on_epoch=True, sync_dist=True)
        self.log('image_dist+', torch.linalg.norm(out[:,1,:,:,:].detach()- out[:,0,:,:,:].detach()), on_step=True, prog_bar=True,
                 on_epoch=True, sync_dist=True)
        self.log('image_dist-', torch.linalg.norm(out[:,1,:,:,:].detach()- out[:,2,:,:,:].detach()), on_step=True, prog_bar=True,
                 on_epoch=True, sync_dist=True)
        ...
        #TODO add samples
        sample_size = 12
        sample_path = self.training_config['sample_path']
        if batch_nb % 25 == 0:
            # print("SAMPLE")
            self.model.eval()
            ...
            utils.save_image(
                ...
                f'./{sample_path}/{str(self.current_epoch + 1).zfill(5)}_{str(batch_nb).zfill(5)}.png',
                nrow=sample_size,
                normalize=True,
                range=(-1, 1),
            )
    ...
    def validation_step(self, batch, batch_nb):
        ...
        self.log('validation_loss', loss, on_step=True,
                 on_epoch=True, sync_dist=True)
        self.log('mse_loss', loss, on_step=True, on_epoch=True, sync_dist=True)
        self.log('latent_loss', loss, on_step=True,
                 on_epoch=True, sync_dist=True)
        self.log('triplet_loss', loss, on_step=True,
                 on_epoch=True, sync_dist=True)
        return loss

    def train_dataloader(self):
        dataset = VideoDataset(self.training_config['path'], split='train',
                               clip_len=self.training_config['clip_length'],
                               clip_jump=self.training_config['skip'],
                               size=self.training_config['size'],
                               preprocess=False,
                               transform=self.transform)
        sampler = DistributedSampler(dataset)
        dataloader_train = DataLoader(
            dataset, batch_size=self.training_config['bs'], num_workers=4, sampler=sampler)
        return dataloader_train

    def val_dataloader(self):
        dataset = VideoDataset(self.training_config['path'], split='val',
                               clip_len=self.training_config['clip_length'],
                               clip_jump=self.training_config['skip'],
                               size=self.training_config['size'],
                               preprocess=False,
